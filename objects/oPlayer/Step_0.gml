...
if(generalstate == playergeneralstate.normal) {
	var addx = 0;
	var addy = 0;
	if(rl!=0||du!=0) {
	...
	var instnear = instance_nearest(x,y,oNPCPar);
	if(keyboard_check_pressed(vk_space)&&distance_to_object(instnear)<32) {
		oDialogueControl.curtext = instnear.lines;
	}
}
depth = -y